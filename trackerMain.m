function [results] = trackerMain(p, im, bg_area, fg_area, area_resize_factor)
    ...
    apces = zeros(num_frames, 1);
    psrs = zeros(num_frames,1);
    max_response = zeros(num_frames, 1);
    ...
    for frame = 1:num_frames
            ...
            apces(frame) = APCE(response);
            psrs(frame) = PSR(response);
            max_response(frame) = max(response(:));
        ...
        if p.visualization == 1
            if frame == 1  %first frame, create GUI
                ...
                rect_handle = rectangle('Position',rect_position,'LineWidth', 2, 'EdgeColor','g');
                rect_padded_handle = rectangle('Position',rect_position_padded, 'LineWidth', 2, 'LineStyle', '--', 'EdgeColor','b');
                ...
                try  %subsequent frames, update GUI
                    ...
                    set(rect_padded_handle, 'Position', rect_position_padded)
    ...
    if p.visualization_apce
        figure()
        subplot(3,1,1)
        plot((1:num_frames), apces)
        title("apce")
        subplot(3,1,2)
        plot((1:num_frames), psrs)
        title("psr")
        subplot(3,1,3)
        plot((1:num_frames), max_response)
        title("max_response")
    end
